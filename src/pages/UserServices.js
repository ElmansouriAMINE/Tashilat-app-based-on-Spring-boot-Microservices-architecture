...
export default function UserServices() {
    ...
    const update = async()=>{
        ...
        axios({
            ...
            url: "http://localhost:8083/user/"+user.id+"/update",
            method: "PUT",
            data : {
                "id":userId,
                "firstName":firstName,
    ...
    const handelDelete = async()=>{
        axios({
            // Endpoint to send files
            url: "http://localhost:8083/user/"+user.id+"/delete/"+userId,
            method: "DELETE",
            })
            // Handle the response from backend here
            .then((res) => {
    ...
    const handelUpgrade = async()=>{
        axios({
            // Endpoint to send files
            url: "http://localhost:8083/user/"+user.id+"/upgrade/"+userId,
            method: "GET",
            })
            // Handle the response from backend here
            .then((res) => {
    ...
    const load = async()=>{
        axios({
            // Endpoint to send files
            url: "http://localhost:8083/user/all/"+user.id,
            method: "GET",
            })
            // Handle the response from backend here
            .then((res) => {
    ...
    return (
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-4">
          ...
          <div style={{display:"none",position:"fixed", right: "40px",top: "80px"}}  class="alert alert-success alert-dismissible" role="alert" id="error">
                ...
                <div class="table-responsive p-3">
                  <table class="table align-items-center table-flush table-hover" id="dataTableHover">
                    ...
                    users.map((user) => (
                        ...
                        {user.role == "admin"?<></>:<td  style={{padding:"12px 6px"}}> <button type="button" class="btn btn-success mb-1" data-dismiss="modal" data-toggle="modal" data-target="#upgrade" onClick={(e)=>{updateUserId(user)}} >upgrade</button></td>}
