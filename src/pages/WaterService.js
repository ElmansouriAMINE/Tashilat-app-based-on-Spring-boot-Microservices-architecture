...
export default class WaterService extends Component {
  ...
  counterNumber() {
    ...
    axios({
      ...
      url: "http://localhost:8083/operation/water/" + this.state.user.id,
      method: "GET",
    })
  ...
  handleUpdate() {
    ...
    axios({
      // Endpoint to send files
      url: "http://localhost:8080/water/update",
      method: "POST",
      data: {
        "id": this.state.rowId,
  ...
  handleUploadP() {
    this.setState({ factureId: document.getElementById('factureId').value }, () => {
      axios({
        ...
        // Endpoint to send files
        url: "http://localhost:8080/water/paye/" + this.state.factureId,
        method: "GET",
      })
  ...
  handleUploadNonP() {
    this.setState({ factureId: document.getElementById('factureId').value }, () => {
      axios({
        ...
        // Endpoint to send files
        url: "http://localhost:8080/water/non paye/" + this.state.factureId,
        method: "GET",
      })
